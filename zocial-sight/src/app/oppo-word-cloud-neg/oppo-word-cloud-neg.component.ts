...
declare let d3: any;
@Component({
  selector: "oppo-word-cloud-neg",
  templateUrl: "./oppo-word-cloud-neg.component.html",
  styleUrls: ["./oppo-word-cloud-neg.component.scss"]
})
export class OppoWordCloudNegComponent implements OnInit {
  @Input()
  wordData;
  data1 = ["Data", "Chea"];
  data = [];
  ...
  ngOnInit() {
    let cls = this;
    this.testService.getAdvantageDataNeg().subscribe(res => {
      this.rawData = res;
      for (let a of this.rawData) {
        this.data.push({ text: a.word, size: a.time });
      }
  ...
  private buildSVG() {
    this.svg = D3.select("div.word-cloud3")
      .append("svg")
