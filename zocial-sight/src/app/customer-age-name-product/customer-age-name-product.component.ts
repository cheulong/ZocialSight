...
@Component({
  selector: 'app-customer-age-name-product',
  templateUrl: './customer-age-name-product.component.html',
  styleUrls: ['./customer-age-name-product.component.scss']
})
export class CustomerAgeNameProductComponent implements OnInit {
  foods = [
...
res1;
  ...
  drawGraph(value) {
    ...
    if (value === ' day ') {
      this.saleService.getForecastRevenue(value).subscribe(res => {
        ...
        if (this.res.length > 0) {
          for (const a in this.res) {
            tempData1.push(res[a].product_list[0].total_revenue);
            data1 = tempData1;
          }
        } else {
          this.data = [];
      ...
      this.saleService.getForecastRevenue(value).subscribe(res => {
        ...
        if (this.res.length > 0) {
          for (const a in this.res) {
            this.cokeRevenue = this.cokeRevenue + res[a].product_list[0].total_revenue;
            if (this.b % 7 === 6) {
  ...
  drawGraph1(value) {
    ...
    this.saleService.getForecastRevenue(value).subscribe(res => {
      ...
      if (this.res.length > 0) {
        for (const a in this.res) {
         tempData2.push(res[a].product_list[1].total_revenue);
          data2 = tempData2;
        }
      } else {
        this.data = [];
     ...
     this.c = 0;
      this.saleService.getForecastRevenue(value).subscribe(res => {
        ...
        if (this.res.length > 0) {
          for (const a in this.res) {
            this.fantaRevenue = this.fantaRevenue + res[a].product_list[1].total_revenue;
            if (this.c % 7 === 6) {
    ...
    this.saleService.getForecastRevenue(value).subscribe(res => {
      ...
      if (this.res.length > 0) {
        for (const a in this.res) {
          ...
          this.tempData.push(res[a].product_list[2].total_revenue);
     ...
     this.d = 0;
      this.saleService.getForecastRevenue(value).subscribe(res => {
        ...
        if (this.res.length > 0) {
          for (const a in this.res) {
            this.spriteRevenue = this.spriteRevenue + res[a].product_list[2].total_revenue;
            if (this.d % 7 === 6) {
